...
document.addEventListener("DOMContentLoaded", () => {
   ...
   function startMultiPlayer() {
      ...
      function playerConnectedOrDisconnected(num) {
         ...
         console.log(
            "in playerConnectedOrDisconnected",
            parseInt(num) === playerNum,
      ...
      startButton.addEventListener("click", (e) => {
         if (shipsContainer.childElementCount == 0) {
            guestGrid.addEventListener("click", (e) =>
               revealSquare(e.target, game, turnsDisplay, guestGrid)
            );
   ...
   hostGrid.addEventListener("drop", (e) => {
      dragDrop(e, target, hostSquares, shipsContainer);
   });
...
function generate(dir, ship, squares) {
   ...
   let randomStart = Math.abs(
      Math.floor(
         Math.random() * squares.length -
            ship.directions["horizontal"].length * direction
      )
   );
   // console.log(randomStart)

   const isTaken = current.some((index) =>
      squares[randomStart + index].classList.contains("taken")
   );
   const isAtRightEdge = current.some(
      (index) => (randomStart + index) % 10 === 9
   );
   ...
   if (!isTaken && !isAtRightEdge && !isAtLeftEdge)
      current.forEach((index) =>
         squares[randomStart + index].classList.add("taken", ship.name, "ship")
      );
...
function dragDrop(e, target, squares, container) {
   ...
   let droppedShipLastId =
      draggedShipLastIndex - draggedShipIndex + receivingSquare;

   let isVertical = [...target.ship.classList].some((className) =>
      className.includes("vertical")
   );

   if (!isVertical) {
      // console.log('it is horizontal');
      let current = shipsArray.find((ship) => ship.name === draggedShipClass)
         .directions.horizontal;
      let isTaken = current.some((index) =>
         squares[droppedShipFirstId + index].classList.contains("taken")
      );
      if (
         Math.floor(droppedShipLastId / 10) ===
            Math.floor(receivingSquare / 10) &&
         !isTaken
      ) {
         console.log(
            "it fits on the same line and none of the squares are already taken"
         );
...
function revealSquare(square, game, turnsDisplay, guestGrid) {
   ...
   if (!square.classList.contains("revealed")) {
      ...
      ).textContent = game.score[game.currentPlayer].total;
      game.currentPlayer = game.currentPlayer === "host" ? "guest" : "host";
      if (game.score[game.currentPlayer].total === 17) gameOver();
      else playGameSingle(game, turnsDisplay, guestGrid);
   }
}
